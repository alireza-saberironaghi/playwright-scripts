...
// const COUNTER_PATH = path.resolve(__dirname, 'counter.json');

// function incrementRunCounter(): number {
//   let currentCount = 0;

//   if (fs.existsSync(COUNTER_PATH)) {
//     const fileData = fs.readFileSync(COUNTER_PATH, 'utf-8');
//     try {
//       const parsed = JSON.parse(fileData);
//       currentCount = parsed.runCount || 0;
//     } catch (e) {
//       console.warn('⚠️ Failed to parse counter.json. Resetting to 0.');
//     }
//   }

//   const newCount = currentCount + 1;
//   fs.writeFileSync(COUNTER_PATH, JSON.stringify({ runCount: newCount }, null, 2));
//   return newCount;
// }
...
const WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbwoaM0YPjIT2hl-Cb0_HrBn24kth5tB9evUub9C3eFnNvd_2rDSJ_EKoGQFiDYULOC3/exec';

function reportRun(testName) {
  ...
  req.on('error', (error) => {
    console.error('❌ Failed to report run:', error.message);
  });
...
// 🌐 Select environment
const environment = Data.environments[0]; // ['qa', 'test', 'sites', dev]
...
const selectOpenAnotherAccount = Data.accountOptions.openAnotherAccount[0]; // ['Yes', 'No']

// 📧 Email and Cell (Manually set for flexibility and external tracking)
const userEmail = 'alex.saber@thirdstream.ca';
const userCell = '6478543392';

const jointUserEmail = 'alex.saberi222@thirdstream.ca';
...
if(user.dob.year > '1930' && user.dob.year < '1939'){
  ...
  await page.getByText(user.dob.day, { exact: true }).first().click();
    ...
    // cell
    await page.getByLabel('Cell #').type(userCell);

    // email
    await page.getByLabel('Email').type(userEmail);
